...
const double acc_x_abs_max = 4000.0;
const double acc_y_abs_max = 4000.0;
...
int main (int argc, char **argv) {
	...
	float data_TSI, theta_x,theta_y,theta_z;
	...
	while (1) {
		...
		if (xdata_fp = fopen(filename, "w")) {
			...
			for (i=0; i<sample; i++) {
				...
				acc_y = data_y;
				acc_z = data_z;
				theta_x =  acos(trunc_norm(acc_x, acc_x_abs_max)) * 180 / PI;
				theta_y =  acos(trunc_norm(acc_y, acc_y_abs_max)) * 180 / PI;
				theta_z =  acos(trunc_norm(acc_z, acc_z_abs_max)) * 180 / PI;

				#ifdef DEBUG
				printf("X command receive: (%f %f %f %f ) t=%d\n", theta_x, theta_y, theta_z, data_TSI, timecycle);
				#endif

				if (i != sample-1) {
					fprintf(xdata_fp, "[%f, %f, %f],\n", theta_x, theta_y, theta_z);
